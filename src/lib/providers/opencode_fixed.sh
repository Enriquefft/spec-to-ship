...
provider_opencode_fixed_invoke() {
    ...
    # Invoke OpenCode with retry - message comes first as positional argument
    local output
    local exit_code
    
    if output=$(_provider_opencode_fixed_retry_with_backoff "${opencode_cmd[@]}" "$message" 2>&1); then
        exit_code=0
    else
        exit_code=$?
    fi
