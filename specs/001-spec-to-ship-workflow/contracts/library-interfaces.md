...
Internal library contracts for `src/lib/*.sh` modules. All functions follow Bash conventions: return 0 on success, non-zero on error, output to stdout, errors to stderr.
...
- **Params**: `phase` - one of: `clarify`, `specs`, `arch`, `plan`, `build`, `gate`, `feedback`
